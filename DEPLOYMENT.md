...
`web` должен стартовать только после того, как `db` перейдёт в состояние `healthy`. Приложение также несколько раз подождёт готовую БД при старте (см. `main.py`), чтобы не падать на первом `connection refused`.

Если после входа в форме снова ошибка, открой логи в момент отправки формы:

```bash
docker compose logs -f web
```

и повтори вход; в логе будет traceback от POST `/auth/login` или `/auth/register`.
